...
def read_pdb(filename, deletewater=False):
    ...
    data["coord"] = np.array([[line[30:38].strip(), line[38:46].strip(
    ), line[46:54].strip()] for line in lines]).astype(np.double)
    data["resid"] = np.array([line[6:11].strip()
...
def get_mask(pdbdata, selection="all"):
    ...
    if selection == "all":
        mask = np.full(
            shape=(len(pdbdata["name"])), fill_value=True).astype(bool)
...
if __name__ == "__main__":
    ...
    RMSD_global = calculate_RMSD(
        reference_geom["coord"][selection_mask], aligned_coords)
    print(RMSD_global)
